...
uint16_t p251p2_add_naive(uint16_t x, uint16_t y) {
  uint8_t* xx = (uint8_t*) &x;
  uint8_t* yy = (uint8_t*) &y;
  uint16_t z0 = (xx[0] + yy[0]) % 251;
  uint16_t z1 = (xx[1] + yy[1]) % 251;
  return z0 | (z1<<8);
}
...
uint32_t p251p4_add_naive(uint32_t x, uint32_t y) {
  ...
  uint16_t z0 = (xx[0] + yy[0]) % 251;
  uint16_t z1 = (xx[1] + yy[1]) % 251;
  uint16_t z2 = (xx[2] + yy[2]) % 251;
  uint16_t z3 = (xx[3] + yy[3]) % 251;
...
uint32_t p251p4_sub_naive(uint32_t x, uint32_t y) {
  uint8_t* xx = (uint8_t*) &x;
  uint8_t* yy = (uint8_t*) &y;
  uint16_t z0 = ((uint16_t)xx[0] + 251 - yy[0]) % 251;
  uint16_t z1 = ((uint16_t)xx[1] + 251 - yy[1]) % 251;
  uint16_t z2 = ((uint16_t)xx[2] + 251 - yy[2]) % 251;
  uint16_t z3 = ((uint16_t)xx[3] + 251 - yy[3]) % 251;
  return z0 | (z1<<8) | (z2<<16) | (z3<<24);
}
...
void create_p251p4_log_tables() {
  ...
  for (uint64_t i = 1; i < SDITH_PP_POWER_P251P4; ++i) {
    ...
    uint16_t ldiff = (lu == SDITH_ORDER_P251P2) ? SDITH_ORDER_P251P2 : lu >= lv ? lu - lv : SDITH_ORDER_P251P2 + lu - lv;
