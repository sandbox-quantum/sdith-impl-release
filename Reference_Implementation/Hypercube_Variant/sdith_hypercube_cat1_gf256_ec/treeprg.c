...
EXPORT void sdith_tree_prg_seed_expand(TREE_PRG_CTX *key, void *out,
                                       const void *in,
                                       const uint64_t first_tweak,
                                       const uint32_t iteration, uint64_t n) {
  ...
  if (n < 8) {
    ...
    for (uint64_t i = 0; i < n / 2; ++i) {
      ...
      sdith_hash(HASH_TREE, out_seeds[i], 2 * PARAM_seed_size, &vec,
                 sizeof(vec));
      ++tweak;
    }
    return;
  ...
  for (uint64_t i = 0; i < n / 2; i += 4) {
    ...
    void *dst4[4] = {out_seeds[i], out_seeds[i + 1], out_seeds[i + 2],
                     out_seeds[i + 3]};
    sdith_hash4(HASH_TREE, dst4, PARAM_seed_size * 2, src4,
                sizeof(seed_expand_vec_t));
  }
}
...
EXPORT void sdith_tree_prg_leaf_expand(TREE_PRG_CTX *key, seed_t in_seed,
  ...
  sdith_hash(HASH_TREE, out, sizeof(out), in, sizeof(in));
...
EXPORT void sdith_tree_prg_leaf_expand4(TREE_PRG_CTX *key, seed_t *in_seed,
  ...
  sdith_hash4(HASH_TREE, outVec, sizeof(out4[0]), inVec, sizeof(in4[0]));
